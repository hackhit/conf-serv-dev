...
# ===============================================
# SCRIPT DE VERIFICACIÓN Y MANTENIMIENTO DEL SERVIDOR
...
check_service() {
    ...
    if systemctl is-active --quiet $service_name; then
        ...
        echo -e "${RED}❌ $service_name: DETENIDO${NC}"
        return 1
    fi
}
...
check_port() {
    ...
    if netstat -tuln | grep -q ":$port "; then
        ...
        echo -e "${RED}❌ Puerto $port ($service): CERRADO${NC}"
        return 1
    fi
}

# Función para verificar sitios web
check_website() {
    local url=$1
    local response=$(curl -s -o /dev/null -w "%{http_code}" $url)
    
    if [ "$response" = "200" ]; then
        echo -e "${GREEN}✅ $url: ACCESIBLE (HTTP $response)${NC}"
        return 0
    else
        echo -e "${RED}❌ $url: ERROR (HTTP $response)${NC}"
        return 1
    fi
}
